...
- **Multi-select not available in Puck.** Alignment, distribution, and size matching operate on all siblings in a container, not a user-selected subset. Puck exposes `selectedItem` (singular). Multi-select would require Puck API changes.
- **Arrow key nudge restricted to free-position containers.** By design — nudging x/y coordinates only applies to absolutely-positioned elements. In flex/grid/stack, position is determined by the layout algorithm.
